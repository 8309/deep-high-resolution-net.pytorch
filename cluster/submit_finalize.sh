...
if [[ $# -lt 1 ]]; then
  echo "Usage: $0 <run_id> [epoch_marker]"
  exit 1
fi
...
BEGIN_EPOCH="${BEGIN_EPOCH:-$EPOCH_MARKER}"
END_EPOCH="${END_EPOCH:-$EPOCH_MARKER}"
...
RUN_POST_DEMO="${RUN_POST_DEMO:-1}"
RUN_MODE="${RUN_MODE:-finalize}"
...
job_id=$(sbatch --parsable \
  --job-name "hrnet-finalize" \
  --output "$RUN_ROOT/slurm/slurm-finalize-%j.out" \
  --error "$RUN_ROOT/slurm/slurm-finalize-%j.err" \
...
echo "[INFO] submitted finalize job: $job_id"
echo "[INFO] run_root: $RUN_ROOT"

echo "job_id=$job_id" > "$RUN_ROOT/summary/finalize_submission.txt"
echo "submitted_utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$RUN_ROOT/summary/finalize_submission.txt"
echo "run_mode=$RUN_MODE" >> "$RUN_ROOT/summary/finalize_submission.txt"
